...
# 将结果写入CSV文件
output_file = 'loop2_output.csv'
with open(output_file, 'w', newline='') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=['File', 'Loop 2 Avg Time'])
    writer.writeheader()
    for filename, times in data.items():
        writer.writerow({'File': filename, 'Loop 2 Avg Time': times['Loop 2 Avg Time']})
